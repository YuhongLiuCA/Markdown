...
class MarkdownExample extends React.Component {
  ...
  handleChange(e) {
    ...
    //const newContent = marked(this.state.content);
    const newContent = $(this.state.content);
    this.setState({ newContent: newContent });
  }

  handleMark() {
    //const newContent = hljs.highlightAuto(this.state.content).value;
    //this.setState({newContent: newContent});
  }
  ...
  render() {
    ...
    let htView = '';
    //if(this.state.input !== '')
    //  htView = parser.parseFromString(this.state.input, 'text/html');
    //let htView = <h3>bg</h3>;
